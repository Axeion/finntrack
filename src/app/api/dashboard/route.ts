...
export async function GET(req: Request) {
  ...
  const af = accountId ? sql` and ${transactions.account_id} = ${accountId}` : sql``
  ...
  const [
    ...
    db
      ...
      .from(transactions)
      .where(sql`cast(${transactions.amount} as numeric) > 0 and ${transactions.posted} >= ${monthStart.toISOString()} and ${transactions.posted} < ${monthEnd.toISOString()}${af}`),
    ...
    db
      .select({ total: sql<string>`coalesce(sum(cast(${transactions.amount} as numeric)), 0)` })
      .from(transactions)
      .where(sql`cast(${transactions.amount} as numeric) > 0 and ${transactions.posted} >= ${priorMonthStart.toISOString()} and ${transactions.posted} < ${priorMonthEnd.toISOString()}${af}`),
    ...
    db
      ...
      .from(transactions)
      .where(sql`cast(${transactions.amount} as numeric) < 0 and ${transactions.posted} >= ${monthStart.toISOString()} and ${transactions.posted} < ${monthEnd.toISOString()} and ${transactions.category} is not null${af}`)
    ...
    db
      ...
      .from(transactions)
      .where(sql`${transactions.posted} >= ${thirtyDaysAgo.toISOString()}${af}`)
      .groupBy(sql`date_trunc('day', ${transactions.posted})::date`)
      .orderBy(sql`date_trunc('day', ${transactions.posted})::date`),

    db.select().from(transactions)
      .where(accountId ? sql`${transactions.account_id} = ${accountId}` : undefined)
  ...
  const totalBalance = selectedAccount
    ...
    : allAccounts.reduce((sum, a) => sum + parseFloat(a.balance), 0)
